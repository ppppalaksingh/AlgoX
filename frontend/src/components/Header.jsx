...
export default function Header({
  ...
  return (
    <header className={`w-full h-20 border-b flex items-center justify-between px-4 sm:px-6 lg:px-8 gap-4 sticky top-0 z-30 transition-all duration-300 backdrop-blur-2xl ${isDark
      ...
      <div className="flex items-center gap-3 shrink-0">
        <button
          onClick={onMenuClick}
          className={`p-2 rounded-xl border transition-colors cursor-pointer shrink-0 ${isDark
        ...
        >
          ...
          <div className="leading-tight">
            <div className="flex items-center gap-1.5">
              <span className={`font-extrabold text-lg font-serif tracking-tight ${isDark ? "text-white group-hover:text-indigo-200" : "text-[#1e143e] group-hover:text-[#5925dc]"
                }`}>
                StatSkill
              </span>
              <span className={`hidden sm:inline-flex text-[9px] font-bold px-2 py-0.5 rounded-full uppercase tracking-widest ${isDark ? "bg-[#5925dc]/20 text-indigo-300 border border-[#5925dc]/30" : "bg-[#5925dc]/10 text-[#5925dc] border border-[#5925dc]/25"
                }`}>
                AI Cadre
              </span>
            </div>
            <span className={`text-[9px] font-bold uppercase tracking-widest block -mt-0.5 ${isDark ? "text-slate-400" : "text-[#7e7298]"
              }`}>
        ...
        {[
          ...
          return (
            <button
              ...
              className={`transition-all relative py-1 cursor-pointer shrink-0 ${isActive
                ? isDark
      ...
      <div className="flex items-center gap-2 sm:gap-3 shrink-0">
        {/* Role Switcher Pill */}
        <div className={`flex items-center p-0.5 rounded-full border shrink-0 ${isDark
          ? "bg-black/30 border-white/[0.08]"
          : "bg-[#ede6da]/70 border-[#e8ded2]"
          }`}>
          <button
            onClick={() => onToggleRole?.("learner")}
            className={`px-3 py-1 rounded-full text-xs font-bold transition-all flex items-center gap-1 cursor-pointer shrink-0 ${currentRole === "learner"
          ...
          <button
            ...
            className={`px-3 py-1 rounded-full text-xs font-bold transition-all flex items-center gap-1 cursor-pointer shrink-0 ${currentRole === "admin"
        ...
        <button
          ...
          className={`w-9 h-9 rounded-full flex items-center justify-center cursor-pointer transition-all duration-300 hover:scale-105 shadow-xs border shrink-0 ${isDark
        ...
        <button
          ...
          className={`hidden sm:flex items-center gap-1.5 px-4 sm:px-5 py-2 text-xs transition-all duration-200 cursor-pointer shrink-0 ${isDark ? "btn-najaba-gold" : "btn-najaba-purple"
            }`}
        ...
        <div ref={searchRef} className="relative shrink-0">
          ...
          {searchOpen && (
            <div className={`absolute right-0 top-12 w-80 rounded-2xl border shadow-2xl overflow-hidden z-50 animate-in fade-in zoom-in-95 duration-100 ${isDark ? "bg-[#1b1242]/98 backdrop-blur-2xl border-white/[0.12]" : "bg-white border-[#e8ded2]"
              }`}>
              <div className="p-3 border-b border-white/[0.06] flex items-center gap-2">
                ...
                <input
                  ...
                  className={`bg-transparent outline-none text-xs w-full ${isDark ? "text-white placeholder:text-slate-500" : "text-[#1e143e] placeholder:text-[#9d94b8]"
                    }`}
                />
                {searchQuery && (
                    ...
                    <div className={`divide-y ${isDark ? "divide-white/[0.05]" : "divide-[#e8ded2]"}`}>
                      {searchResults.map((course) => (
                        <div
                          ...
                          className={`p-3 cursor-pointer transition-colors ${isDark ? "hover:bg-white/[0.05]" : "hover:bg-[#faf7f2]"
                            }`}
                        >
        ...
        <div ref={notifRef} className="relative shrink-0">
          <button
            ...
            className={`relative p-2 rounded-xl cursor-pointer transition-colors border shrink-0 ${isDark
              ? "text-slate-300 hover:text-white hover:bg-white/[0.06] border-white/10"
          ...
          {notificationsOpen && (
            <div className={`absolute right-0 top-12 w-80 rounded-2xl border shadow-2xl z-50 overflow-hidden animate-in fade-in zoom-in-95 duration-100 ${isDark ? "bg-[#1b1242]/95 backdrop-blur-2xl border-white/[0.1]" : "bg-white border-[#e8ded2]"
              }`}>
              <div className={`p-3.5 border-b flex items-center justify-between ${isDark ? "border-white/[0.08]" : "border-[#e8ded2] bg-[#faf7f2]"
                }`}>
        ...
        <div ref={profileRef} className="relative shrink-0">
          <button
            ...
            className={`flex items-center gap-2 p-1.5 sm:px-2.5 rounded-xl border transition-all cursor-pointer shrink-0 ${isDark
          ...
          {profileDropdownOpen && (
            <div className={`absolute right-0 top-12 w-64 rounded-2xl border shadow-2xl z-50 p-2 overflow-hidden animate-in fade-in zoom-in-95 duration-100 ${isDark ? "bg-[#1b1242]/95 backdrop-blur-2xl border-white/[0.1]" : "bg-white border-[#e8ded2]"
              }`}>
              <div className={`p-3 border-b mb-1 rounded-xl ${isDark ? "border-white/[0.08] bg-white/[0.03]" : "border-[#e8ded2] bg-[#faf7f2]"
                }`}>
              ...
              <button
                ...
                className={`w-full flex items-center gap-2.5 px-3 py-2 text-xs font-medium rounded-xl transition-colors cursor-pointer ${isDark ? "text-slate-300 hover:text-white hover:bg-white/[0.06]" : "text-[#4a3e65] hover:text-[#1e143e] hover:bg-[#faf7f2]"
                  }`}
              >
